    ...
    if isinstance(value, list):
        ...
        if {"text", "start", "end"} <= value.keys():
            ...
            if (
                ...
                or source[value["start"] : value["end"]] != value["text"]
            ):
...
def validate_structured_output(value: Any, schema: dict[str, Any], payload: dict[str, Any]) -> None:
...
def _fake_chat(task: str, payload: dict[str, Any]) -> Any:
    if task == "highlighting":
        words = set(
            re.findall(r"[a-z0-9]{4,}", str(payload.get("description_verbatim", "")).lower())
        )
        ranked = [
            (len(words & set(re.findall(r"[a-z0-9]{4,}", s["text"].lower()))), s["id"])
            for s in payload.get("segments", [])
        ]
        return {
            "sentence_ids": [sid for score, sid in sorted(ranked, reverse=True) if score > 0][:5]
        }
    if task == "extraction":
        text = str(payload.get("description_verbatim", ""))
        return {
            "claims": _span_matches(text, r"[^.!?\n]+(?:[.!?]|$)", "claim")[:32],
...
class ModelGateway:
    ...
    async def _request(
        self, profile: ModelProfile, path: str, body: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def structured(self, task: str, payload: dict[str, Any]) -> GatewayResponse:
        ...
        if self.fake:
            ...
            if profile.structured_output == "json_object":
                system_content += "\n\nJSON Schema:\n" + json.dumps(
                    prompt.response_schema, separators=(",", ":")
                )
            body = {
                "model": profile.model,
                "messages": [
    ...
    async def embed(
        self, task: str, texts: list[str], purpose: EmbeddingPurpose
    ) -> EmbeddingResponse:
        ...
        prepared = [
            profile.query_prefix + text if purpose == EmbeddingPurpose.QUERY else text
            for text in texts
        ]
        if self.fake:
            ...
            result = await self._request(
                profile, "/embeddings", {"model": profile.model, "input": prepared}
            )
            vectors = [
                row["embedding"]
                for row in sorted(result["data"], key=lambda row: row.get("index", 0))
            ]
        ...
        return EmbeddingResponse(
            ...
            | {"purpose": purpose.value, "query_prefix_applied": purpose == EmbeddingPurpose.QUERY},
        )
...
def create_app() -> FastAPI:
    ...
    gateway = ModelGateway(
        ModelRegistry.load(settings.model_config_path, settings.prompt_root),
