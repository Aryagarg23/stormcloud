...
from .models import OutboxEvent, ProcessingStage, StageStatus
from .observability import configure_logging
from .pipeline import build_ready_bundles, build_signal_evidence, mark_stage, process_embedding, process_extraction, process_fetch, process_graph, process_highlighting, process_nlp, send_invitation
        ...
        try:
            with session_scope() as session:
                rows = session.scalars(select(OutboxEvent).where(
                    ...
                    .order_by(OutboxEvent.created_at).limit(settings.worker_batch_size)
                    .with_for_update(skip_locked=True)).all()
                for row in rows:
                    payload = dict(row.payload)
                    envelope = EventEnvelope(
                        event_id=str(row.event_id), schema_version=row.schema_version,
                        subject=row.subject, aggregate_type=row.aggregate_type,
                        aggregate_id=str(row.aggregate_id),
                        correlation_id=str(row.correlation_id),
                        causation_id=str(row.causation_id) if row.causation_id else None,
                        occurred_at=row.created_at.isoformat(), payload=payload)
                    await js.publish(row.subject, envelope.encode(),
...
def processor_for(role: str, subject: str) -> Processor:
    ...
    if role == "nlp":
        return process_nlp
    if role == "llm":
        return process_highlighting if subject == "stormcloud.document.ready.v1" else process_extraction
    if role == "embedding":
    ...
    if role == "controller":
        async def coordinate(session, event):
            if event.subject == "stormcloud.signal.embedding.ready.v1":
...
SUBJECTS = {
    ...
    "llm": ["stormcloud.signal.submitted.v1", "stormcloud.document.ready.v1"],
    ...
    "controller": ["stormcloud.researcher.ready.v1", "stormcloud.nlp.ready.v1",
                   "stormcloud.highlights.ready.v1", "stormcloud.highlight.changed.v1",
...
async def mark_dead_letter(event: EventEnvelope, role: str, error: Exception) -> None:
    with session_scope() as session:
        ...
        if not operation_id:
            return
        mark_stage(session, event.payload, role, StageStatus.dead_lettered,
...
async def subscribe_role(js, role: str) -> None:
    async def callback(message):
        ...
        try:
            ...
            log.exception("event.failed", role=role, subject=message.subject,
                          event_id=event.event_id, delivered=delivered, error=str(exc))
...
async def run() -> None:
    ...
    connection = await nats.connect(settings.nats_url, name=f"stormcloud-{role}",
                                    reconnect_time_wait=2, max_reconnect_attempts=-1)
    js = connection.jetstream()
    try:
        await js.add_stream(config=StreamConfig(name=settings.nats_stream,
            subjects=["stormcloud.>"], storage="file", duplicate_window=120))
    except Exception as exc:
        if "stream name already in use" not in str(exc).lower() and "already in use" not in str(exc).lower():
