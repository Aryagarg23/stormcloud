...
def assemble_signal_evidence(*, signal_id: str, description_verbatim: str,
                             document_id: str, document_text: str,
                             ...
                             recipe_version: str = "signal-evidence-v1") -> EvidenceSnapshot:
    sources = {f"signal:{signal_id}:description": description_verbatim,
    ...
    manifest = {"kind": "signal", "signal_id": signal_id, "document_id": document_id,
                ...
                "extraction": extraction or {}, "provenance": provenance or {},
                "recipe_version": recipe_version}
    evidence_text = "\n\n".join(sections)
    envelope = _canonical({"manifest": manifest, "evidence_text": evidence_text})
    digest = hashlib.sha256(envelope).hexdigest()
    return EvidenceSnapshot(recipe_version=recipe_version, evidence_text=evidence_text,
                            manifest=manifest, content_sha256=digest,
                            object_key=content_addressed_key("evidence/signals", envelope))

def assemble_bundle_evidence(*, bundle_id: str, members: Sequence[BundleMemberEvidence],
                             thesis: str | None = None, ordered: bool = False,
                             provenance: dict[str, Any] | None = None,
                             recipe_version: str = "bundle-evidence-v1") -> EvidenceSnapshot:
    ...
    sections.extend(f"MEMBER {m.position + 1} [{m.signal_id}]\n" +
                    (f"NOTE: {m.note}\n" if m.note else "") + m.evidence_text for m in members)
    manifest = {"kind": "bundle", "bundle_id": bundle_id, "thesis": thesis,
                "ordered": ordered, "members": [m.model_dump(exclude={"evidence_text"})
                for m in members], "provenance": provenance or {},
                "recipe_version": recipe_version}
    evidence_text = "\n\n".join(sections)
    envelope = _canonical({"manifest": manifest, "evidence_text": evidence_text})
    return EvidenceSnapshot(recipe_version=recipe_version, evidence_text=evidence_text,
                            manifest=manifest, content_sha256=hashlib.sha256(envelope).hexdigest(),
