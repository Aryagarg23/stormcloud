...
def remember(db: Session, user: User, key: str | None, body: BaseModel, result: Accepted):
    if key:
        db.add(
            IdempotencyRecord(
                user_id=user.id,
...
def refresh(body: RefreshRequest, db: DB):
    old = db.scalar(
        ...
        .where(RefreshSession.token_hash == token_hash(body.refresh_token))
        .with_for_update()
    )
...
def logout(body: RefreshRequest, db: DB):
    row = db.scalar(
        select(RefreshSession).where(RefreshSession.token_hash == token_hash(body.refresh_token))
    )
...
@router.post("/auth/accept-invite", response_model=TokenPair, include_in_schema=False)
@router.post("/auth/invitations/accept", response_model=TokenPair)
...
def invite(body: InvitationCreate, admin: AdminUser, db: DB):
    ...
    queue(
        db,
        "mail.invitation.requested",
        "invite",
        "invitation",
        invitation.id,
        admin.id,
        {"email": invitation.email, "invite_url": url},
    )
    db.commit()
    db.refresh(invitation)
    return InvitationIssued(
        **InvitationView.model_validate(invitation).model_dump(),
        invite_url=url,
        token=raw if settings.debug_return_invite_token else None,
    )
...
def update_user(user_id: UUID, body: UserPatch, admin: AdminUser, db: DB):
    ...
    if user.id == admin.id and (body.is_active is False or body.role == Role.member):
        fail(409, "You cannot deactivate or demote your own administrator account")
...
def create_bundle(
    ...
    for position, item in enumerate(body.items):
        ...
        db.flush()
        db.add(
            BundleItem(bundle_id=bundle.id, signal_id=signal.id, position=position, note=item.note)
        )
...
def list_bundles(
    user: CurrentUser, db: DB, include_archived: bool = False, limit: int = 50, offset: int = 0
):
    query = (
        select(Bundle)
        .options(selectinload(Bundle.items).selectinload(BundleItem.signal))
...
def bundle_neighbors(bundle_id: UUID, user: CurrentUser, db: DB, limit: int = 25):
    return list(
        db.scalars(
            select(Edge)
            .where(
                Edge.source_type == "bundle",
                Edge.source_id == bundle_id,
                Edge.kind == EdgeKind.similarity,
            )
            .order_by(Edge.weight.desc())
            .limit(min(limit, 100))
...
def get_document(version_id: UUID, user: CurrentUser, db: DB):
    ...
    text = (
        ObjectStore()
        .get_bytes(get_settings().s3_bucket_normalized, row.normalized_object_key)
        .decode()
    )
...
def list_highlights(signal_id: UUID, user: CurrentUser, db: DB):
    return list(
        db.scalars(
            select(Highlight)
            .where(Highlight.signal_id == signal_id, Highlight.tombstoned_at.is_(None))
...
def add_highlight(signal_id: UUID, body: HighlightCreate, user: CurrentUser, db: DB):
    ...
    if not signal or not signal.document_version_id:
        fail(
            409 if signal else 404,
            "Signal has no normalized document yet" if signal else "Signal not found",
        )
    document = db.get(DocumentVersion, signal.document_version_id)
    text = (
        ObjectStore()
        .get_bytes(get_settings().s3_bucket_normalized, document.normalized_object_key)
        .decode()
    )
    if (
        body.end_offset > len(text)
        or text[body.start_offset : body.end_offset] != body.text_verbatim
    ):
    ...
    highlight = Highlight(
        ...
        text_verbatim=body.text_verbatim,
...
@router.delete(
    "/signals/{signal_id}/highlights/{highlight_id}", response_model=Accepted, status_code=202
)
...
@router.post(
    "/signals/{signal_id}/auto-highlights/{highlight_id}/suppress",
...
def suppress(signal_id: UUID, highlight_id: UUID, user: CurrentUser, db: DB):
    ...
    if (
        not highlight
        or highlight.signal_id != signal_id
        or highlight.kind != HighlightKind.automatic
    ):
        fail(404, "Automatic highlight not found")
    row = db.scalar(
        select(HighlightSuppression).where(
    ...
    if row:
        ...
        db.add(
            HighlightSuppression(
                signal_id=signal_id, highlight_id=highlight_id, created_by_id=user.id
            )
        )
    operation = queue(
        db,
...
def operations(
    user: CurrentUser, db: DB, status_filter: OperationStatus | None = None, limit: int = 50
):
    query = (
        select(Operation)
        .options(selectinload(Operation.stages))
...
def dead_letters(admin: AdminUser, db: DB, limit: int = 100):
    return list(
        db.scalars(
            ...
            .where(ProcessingStage.status == StageStatus.dead_lettered)
...
def retry_dead_letter(stage_id: UUID, admin: AdminUser, db: DB):
    ...
    if not original:
        fail(404, "Original operation not found")
    subject = f"{original.aggregate_type}.retry.requested"
    operation = queue(
        db,
        subject,
        "retry",
        ...
        admin.id,
        {"retry_of_operation_id": str(original.id), "stage": stage.name},
    )
    db.flush()
    db.add(
        ProcessingStage(
