...
def _segment(
    kind: Literal["paragraph", "sentence"], text: str, start: int, end: int
) -> TextSegment:
    return TextSegment(
        id=_id(kind, start, end, text[start:end]),
        kind=kind,
        start=start,
        end=end,
        text=text[start:end],
...
def analyze_text(raw: str) -> NLPResult:
    ...
    for kind, pattern in patterns:
        for match in re.finditer(pattern, text):
            features.append(
                FeatureSpan(
                    id=_id(kind, *match.span(), match.group()),
                    kind=kind,
                    start=match.start(),
                    end=match.end(),
                    text=match.group(),
